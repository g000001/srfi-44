...
(defun make-generic/constraint (&rest classes)
  (let ((g (make-instance 'standard-generic-function)))
    (cl:add-method g
                   (make-instance 'standard-method
                                  :function (lambda (args nexts)
                                              (declare (ignore nexts args))
                                              (apply #'error "Can't apply to anything but with arguments of"
                                                     classes ))))
    g ))

#|(let ((g (make-generic/constraint <collection>)))
  (funcall g 8))|#
...
(define-syntax define-constrained-generics
  (syntax-rules ()
    ((define-constrained-generics ?class ?generic)
     (define-function ?generic (make-generic/constraint ?class)))
